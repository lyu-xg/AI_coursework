...
def enhancedFeatureExtractorDigit(datum):
    ...
    def somethingLeft(x,y):
        return sum([datum.getPixel(i,y) for i in range(x)]) > 0
    def somethingRight(x,y):
        return sum([datum.getPixel(i,y) for i in range(x,DIGIT_DATUM_WIDTH)]) > 0 
    def somethingTop(x,y):
        return sum([datum.getPixel(x,i) for i in range(y)]) > 0 
    def somethingBotton(x,y):
        return sum([datum.getPixel(x,i) for i in range(y,DIGIT_DATUM_HEIGHT)]) > 0
    def selfZero(x,y):
        #surrounding = [datum.getPixel(x,y),datum.getPixel()]
        return datum.getPixel(x,y) == 0
    def traped(x,y):
        return selfZero(x,y) and  somethingLeft(x,y) and  somethingRight(x,y) and  somethingTop(x,y) and  somethingBotton(x,y)
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        ...
        for y in range(DIGIT_DATUM_HEIGHT):
            ...
            #if traped(x,y):
            #    features["trap"] += 1
        #features[(x,11,"x")]=count1
        #features[(x,22,"x")]=count2
        features[(x,1,"x")] = 1 if count1>5 else 0
        features[(x,2,"x")] = 1 if count2>10 else 0
    #features["x1count"]=sum([features[(i,1,"x")] for i in range(DIGIT_DATUM_WIDTH)])
    #features["x2count"]=sum([features[(i,2,"x")] for i in range(DIGIT_DATUM_WIDTH)])
    '''
    if features["trap"] < 10:
        features["trap"] = 0
    elif features["trap"]>=10 and features["trap"] < 20:
        features["trap"] = 1
    elif features["trap"]>=20 and features["trap"] < 30:
        features["trap"] = 2
    else:
        features["trap"] =3
    '''
    ...
    for y in range(DIGIT_DATUM_HEIGHT):
        ...
        for x in range(DIGIT_DATUM_WIDTH):
            ...
            if pixel == 2:
                count2+=1
    '''
        #features[(y,11,"y")]=count1
        #features[(y,22,"y")]=count2
        #features[(y,1,"y")] = 1 if count1>5 else 0
        #features[(y,2,"y")] = 1 if count2>10 else 0
    #features["y1count"]=sum([features[(i,1,"y")] for i in range(DIGIT_DATUM_HEIGHT)])
    #features["y2count"]=sum([features[(i,2,"y")] for i in range(DIGIT_DATUM_HEIGHT)])
...
def enhancedPacmanFeatures(currentGameState, action):
    """
    ...
    def ghostThreateness(ghostPos, ghostDir, scaredTimer):
        scaryness = manD(ghostPos, newPos)
        if scaredTimer > 3:
            return 0
        if scaryness <= 2:
            return 1
    ...
    features["lose"]=currentGameState.isLose()
    features["win"]=currentGameState.isWin()
    for i,(l,d,s) in enumerate(ghostStates):
        features[("ghostDis",i)]= ghostThreateness(l,d,s)

    features["cloestGhost"] = min([manD(ghostPos, newPos) for ghostPos,d,s in ghostStates]) <= 2

    features["nearestDistance"] = min([manD(newPos,food) for food in foodList]) if len(foodList)>0 else 100
    features["score"]=currentGameState.getScore()
    #features["findFoodCost"]=findFoodCost()
