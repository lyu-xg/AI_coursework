...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        positions = iter(self.legalPositions)
        for i in range(self.numParticles%len(self.legalPositions)):
            self.particles.append(positions.next())
    ...
    def observe(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            if emissionModel[trueDistance] > 0:
                weightedParticles[particle] += emissionModel[trueDistance]

        if weightedParticles == util.Counter():
            self.initializeUniformly(gameState)
        else:
            self.particles = []
            for i in range(self.numParticles):
                self.particles.append(util.sample(weightedParticles))
    ...
    def initializeParticles(self):
        ...
        def loopingGenerator(lst):
            ...
            while True:
                ...
                yield lst[num]
                num+=1
        possibleLayouts = itertools.product(self.legalPositions,repeat=self.numGhosts)
        legalLayouts = [l for l in possibleLayouts if len(set(l)) == self.numGhosts]
        x = loopingGenerator(legalLayouts)
        for i in range(self.numParticles):
            self.particles.append(x.next())
        ...
        for particle in self.particles:
            ...
            for ghostIndex in range(self.numGhosts):
                ...
                if noisyDistance == None:
                    ...
                    base *= emissionModel[trueDistance]
            weightedParticles[particle] += base

        if weightedParticles == initialWeight:
            self.initializeParticles()
            for ghostIndex in range(self.numGhosts):
                if noisyDistances[ghostIndex] == None:
                    self.particles = [self.getParticleWithGhostInJail(particle, ghostIndex) for particle in self.particles]
        else:
