...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        Q = sum([prob*(R(state,action,nextState) + self.discount*self.getValue(nextState))
    ...
    def computeActionFromValues(self, state):
        ...
        if self.mdp.isTerminal(state) or len(legalActions)==0:
            return None
        lst = [(index,self.computeQValueFromValues(state,action)) for index,action in enumerate(legalActions)]
        actionIndex, Q = max(lst,key = lambda x:x[1])
